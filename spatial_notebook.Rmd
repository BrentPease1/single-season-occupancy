...
psi1 <- plogis(beta0 + values(Xraster))
...
```
Call WinBUGS from R
```{r call WinBUGS from R for imperfect CAR}
setwd(here("bugs_output"))
out.imp.car <- bugs(data = imp.car.data, inits = car.inits, parameters.to.save = car.parameters, model.file = "imperfect_CAR.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, bugs.dir = BD, working.directory = getwd(), DIC = FALSE, debug = FALSE)
out.car
```
